...
**任务清单**：
- [x] 后端：创建分类器服务 `app/services/etf_classifier.py`
- [x] 后端：实现规则匹配引擎和完整关键词库
- [x] 测试：编写参数化单元测试，覆盖验证矩阵全部用例
- [x] 验证：用全量 ETF 列表跑分类，输出结果
- [x] 验证：人工审核分类结果，调整规则
- [x] 验证：统计准确率，确认 ≥ 85%
- [x] 测试：边界测试（空名称、特殊字符、超长名称）
- [x] 测试：性能测试（全量 ETF 分类 < 500ms）

**验收结果**：
- 验证矩阵 31 个用例全部通过
- 全量 93 只 ETF 分类覆盖率 100%
- 全量分类性能 3ms，远超目标
...
**任务清单**：
- [x] 后端：集成分类器到 `akshare_service.py`，在获取 ETF 信息时自动分类（`_enrich_with_tags()`）
- [x] 后端：修改 `/etf/search` 接口，响应中增加 `tags` 字段
- [x] 后端：修改 `/etf/batch-price` 接口，响应中增加 `tags` 字段
- [x] 后端：修改 `/etf/{code}/info` 接口，补充 `tags` 字段（缓存优先，缓存无则实时分类）
- [x] 前端：`StockCard` 组件支持标签展示（`showTags` prop，最多显示 2 个）
- [x] 前端：详情页展示完整标签列表
- [x] 前端：标签配色方案 `lib/tag-colors.ts`（按 group 分组着色）
- [x] 测试：端到端验证分类标签正确展示
- [x] 文档：更新 API 文档和 AGENTS.md

**实际实现细节**：
- 分类器在 `akshare_service.py` 的 `_enrich_with_tags()` 中调用，数据写入内存缓存前完成计算
- DiskCache 不存储 tags，冷启动恢复时重新计算
- `/etf/{code}/info` 端点增加了 fallback 逻辑：缓存中无 tags 时实时调用 `ETFClassifier().classify()`
- 前端标签配色：type=蓝色、industry=紫色、strategy=琥珀色、special=灰色
- `StockCard` 通过 `showTags={true}` 控制标签显示，卡片最多展示 2 个标签

**验收结果**：
- 分类准确率 ≥ 85%（全量覆盖率 100%）
- 标签计算性能 < 1ms
- 搜索结果、批量价格、详情页均正确展示标签
...
**设计调整说明**：
实际实现相比原始设计做了简化，采用更轻量的方案（详见 `docs/implementation/2026-02-11-etf-classifier-stage2-impl.md`）：

| 原始设计 | 实际实现 | 调整理由 |
|---------|---------|---------|
| `GET /etf/tags` 返回分组标签 | `GET /etf/tags/popular` 返回硬编码热门标签 | 热门标签足够覆盖主要使用场景，无需动态聚合 |
| `GET /etf/search-by-tags` 独立端点 | 复用 `GET /etf/search?tag=xxx` | 减少端点数量，tag 参数与 q 参数互斥 |
| 多选标签（OR 逻辑） | 单选标签 | 交互更简洁，点一个看结果再切换 |
| 级联筛选（type → industry） | 单行热门标签横向滚动 | 移动端体验更好，减少操作步骤 |

**任务清单**：
- [x] 后端：`cache.py` 新增 `filter_by_tag()` 方法，按标签 label 线性扫描内存缓存
- [x] 后端：新增 `GET /etf/tags/popular` 端点，返回 14 个精选热门标签
- [x] 后端：修改 `GET /etf/search`，`q` 改为可选，新增 `tag` 参数（tag 优先于 q）
- [x] 前端：搜索页添加标签行 UI（横向滚动、`TAG_COLORS` 着色）
- [x] 前端：三态状态机（idle / tag-filtering / text-searching）
- [x] 前端：AbortController 防止标签快速切换时的竞态条件
- [x] 前端：无障碍支持（`role="radiogroup"` + `aria-checked`）
- [x] 测试：后端 13 个测试（`filter_by_tag` 单元测试 + API 集成测试）
- [x] 测试：前端 6 个测试（标签渲染、点击交互、互斥逻辑、无障碍属性）
- [x] 文档：更新 AGENTS.md API 速查表、实施计划状态

**实际实现细节**：
- 热门标签列表硬编码 14 个标签，混合 type/industry/strategy 三种分组
- 搜索页三态互斥：搜索框为空时显示标签行，输入文字后隐藏标签行
- 标签选中态使用 `bg-primary text-primary-foreground`，未选中态按 group 着色
- 结果标题动态切换：标签筛选显示「{tag}」相关，文本搜索显示"搜索结果"
- 空状态文案区分：标签筛选显示 `未找到「{tag}」相关的 ETF`

**涉及文件**：
- `backend/app/core/cache.py` — 新增 `filter_by_tag` 方法
- `backend/app/api/v1/endpoints/etf.py` — 新增 `/tags/popular`，修改 `/search`
- `frontend/app/search/page.tsx` — 标签状态、标签行 UI、统一搜索逻辑
- `backend/tests/api/test_etf_tag_search.py` — 后端测试
- `frontend/__tests__/search-tag-filter.test.tsx` — 前端测试
...
| 2026-02-11 | v1.5 | 阶段1完成：分类器集成到 akshare_service.py；搜索/批量价格/详情接口均返回 tags 字段；StockCard 支持标签展示；详情页展示完整标签；新增 tag-colors.ts 配色方案 |
| 2026-02-11 | v1.6 | 阶段2完成：搜索页标签筛选功能。实现简化为单选热门标签（原设计为多选级联筛选）；复用 /search?tag= 参数（原设计为独立 /search-by-tags 端点）；新增 /tags/popular 端点、filter_by_tag 缓存方法、三态状态机、AbortController 竞态防护；后端 13 测试 + 前端 6 测试全部通过 |
