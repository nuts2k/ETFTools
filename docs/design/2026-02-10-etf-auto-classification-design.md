...
实现：
- 在 ETFCacheManager.set_etf_list() 中调用分类器（缓存写入时计算）
- 标签随 ETF 基础信息一起写入 etf_map，后续读取无需重复计算
- 搜索和批量价格接口直接从缓存读取已计算好的标签
...
相似度算法：
- 标签交集大小 / 标签并集大小（Jaccard 相似度）
- industry 标签匹配权重最高（×2）
- type 和 strategy 标签正常权重（×1）
- 排除自身
- 按相似度降序排序
```
...
**验证措施**：
- 4.2.4 节定义了 19 个关键边界用例的验证矩阵，阶段1完成后必须全部通过
- 阶段1完成后抽样100个 ETF 验证整体准确率 ≥ 85%
