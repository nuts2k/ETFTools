...
- **实现思路**:
  - **依赖**: 基于 ETF 分类标签系统（功能#9）的行业分类数据
  - 热力图组件：矩形面积代表规模，颜色代表涨跌幅
  - 支持按日/周/月切换时间维度
  - 点击板块可展开查看该分类下的具体 ETF
- **复杂度**: 中
- **优先级**: P2
- **前置依赖**: 功能#9（ETF 分类标签系统）
...
- **描述**: 系统自动识别 ETF 的行业、主题、风格属性，并支持用户手动分组管理
- **用户价值**:
  - 降低 ETF 选择门槛，快速理解市场结构
  - 支持按行业/主题筛选和分类浏览
  - 自选列表自动分组，清晰管理持仓结构
- **实现思路**:
  - **阶段1：自动分类（基础设施）**
    - 基于 ETF 名称的规则匹配引擎
    - 多维度标签：指数类型、行业分类、投资风格、特殊属性
    - 搜索结果和详情页展示标签
    - 支持按标签筛选和分类浏览
  - **阶段2：用户分组（个性化管理）**
...
| 9 | ETF 分类标签系统 | 中 | 基础设施功能，支撑分类浏览、热力图等上层功能 |
...
第一批（P0）: 每日摘要推送 → 到价提醒 → ETF 对比 → 定投回测
     ↓
第二批（P1）: 成交量图表 → 资金流向 → ETF 分类标签系统（阶段1：自动分类） → 相关性分析
     ↓
第三批（P1）: ETF 分类标签系统（阶段2：用户分组） → 持仓组合管理
     ↓
第四批（P2）: 热力图（依赖分类系统） / 分红记录 / 笔记 / 分享
```

**说明**：
- ETF 分类标签系统分两阶段实施，阶段1（自动分类）是基础设施，优先实现
- 板块/行业热力图依赖于分类标签系统，需在分类系统完成后实施

---

## 更新日志

- **2026-02-10**: 更新功能#9为"ETF 分类标签系统"，整合自动分类和用户分组功能；更新功能#4添加依赖说明；调整实施路径
